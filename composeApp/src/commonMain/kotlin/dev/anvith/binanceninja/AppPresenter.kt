...
class AppPresenter(
  ...
  private fun fetchCurrencies() {
    launch {
      ...
      updateState { it.copy(userCurrency = userCurrency, errorMessage = null, isLoading = true) }
      currencyRepository
        .getAllFiatCurrencies(forceRefresh = true) { error ->
          logE("Error While fetching Fiat currencies", throwable = error)
          updateState { it.copy(errorMessage = errorHandler.getMessage(error), isLoading = false) }
        }
        .collectLatest {
          updateState { state ->
            state.copy(
