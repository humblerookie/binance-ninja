...
class CurrencyRepository(
    ...
    suspend fun saveUserCurrency(currency: CurrencyModel) = withContext(dispatcherProvider.io()) {
    ...
    suspend fun getUserCurrency() = withContext(dispatcherProvider.io()) {
        queries.getDefaultFiatCurrency().executeAsOneOrNull()?.let(mapper::toDomain)
            ?: defaultUserCurrency()
    }

    fun userCurrencyUpdates() =
        userCurrencyUpdates.asStateFlow().onStart { emit(getUserCurrency()) }

    private fun defaultUserCurrency() = CurrencyModel(
    ...
    private suspend fun fetchRemoteCryptoCurrencies(onSyncFailure: (Throwable) -> Unit) {
        ...
        api
            ...
            .whenSuccess { response ->
                try {
                    val currencies = response.data.areas
                        .first { it.area == P2P }
                        .tradeSides
                        .flatMap { it.assets }
                        .toSet()
